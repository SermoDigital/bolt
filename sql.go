...
func (dc *driverConn) prepareLocked(query string) (stmt, error) {
	...
	if err == nil {
		...
		//
		// TODO(bradfitz): after Go 1.2, closing stmts
...
// driverStmt associates a stmt with the
...
func openPool(drv opener, dataSourceName string) (*DB, error) {
	db := &DB{
		driver:   drv,
		dsn:      dataSourceName,
...
func OpenPool(driverName, dataSourceName string) (*DB, error) {
	...
	// Must not be logging.
	case recorderName:
		return openPool(&recorder{}, dataSourceName)
	case boltName:
		return openPool(&drv{}, dataSourceName)
	default:
		return nil, errors.New("unrecognized (misspelled?) driver")
	}
}
...
func (db *DB) openNewConnection() {
	...
	ci, err := db.driver.OpenNeo(db.dsn)
	db.mu.Lock()
	defer db.mu.Unlock()
	if db.closed {
		if err == nil {
...
// already closed.
func (db *DB) noteUnusedDriverStatement(c *driverConn, si stmt) {
	db.mu.Lock()
	defer db.mu.Unlock()
	if c.inUse {
...
func (db *DB) exec(query string, args map[string]interface{}, strategy connReuseStrategy) (res Result, err error) {
	...
	dc.Lock()
	resi, err := dc.ci.Exec(query, args)
	dc.Unlock()
	if err != nil {
		return nil, err
	}
...
func (db *DB) query(query string, args map[string]interface{}, strategy connReuseStrategy) (*Rows, error) {
...
func (db *DB) queryConn(dc *driverConn, releaseConn func(error), query string, args map[string]interface{}) (*Rows, error) {
	dc.Lock()
	rowsi, err := dc.ci.Query(query, args)
	dc.Unlock()
	if err != nil {
		releaseConn(err)
		return nil, err
...
type stmt interface {
	Close() error
	NumInput() int
	Exec(map[string]interface{}) (Result, error)
	Query(map[string]interface{}) (rows, error)
}
...
func resultFromStatement(ds driverStmt, args map[string]interface{}) (Result, error) {
...
func (s *Stmt) connStmt() (ci *driverConn, releaseConn func(error), si stmt, err error) {
...
func rowsiFromStatement(ds driverStmt, args map[string]interface{}) (rows, error) {
	ds.Lock()
	want := ds.si.NumInput()
	ds.Unlock()
...
func (dr driverResult) Metadata() map[string]interface{} {
	dr.Lock()
	defer dr.Unlock()
	return dr.resi.Metadata()
}
